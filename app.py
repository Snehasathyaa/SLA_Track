...
# ── Complaint wrapper ──────────────────────────────────────────────────────────
    ...
    def sla_violated(self):
        if self.status in ('Resolved', 'Closed'):
            return False
        return self.days_pending > self.sla_threshold
    @property
...
def register():
    if request.method == 'POST':
        ...
        department = request.form.get('department','') if role == 'official' else ''
        if role == 'official' and not department:
            flash('Please select a department for official accounts.','danger')
            return redirect(url_for('register'))
        if query_db('SELECT id FROM users WHERE email=?',[email],one=True):
            flash('Email already registered.','danger')
            return redirect(url_for('register'))
        modify_db('INSERT INTO users (name,email,password,role,district,department) VALUES (?,?,?,?,?,?)',
                  [name,email,generate_password_hash(pw),role,district,department])
        flash('Account created! Please log in.','success')
        return redirect(url_for('login'))
    return render_template('register.html', districts=DISTRICTS,
                           departments=sorted(set(DEPARTMENTS.values())))
...
def update_complaint(cid):
    ...
    # Fetch current row so we preserve existing values
    row = query_db('SELECT * FROM complaints WHERE id=?', [cid], one=True)
    if not row:
        flash('Complaint not found.', 'danger')
        return redirect(request.referrer or url_for('official_dashboard'))

    # resolved_at: set when first resolved, never cleared
    cur_resolved = row['resolved_at']
    resolved_at  = cur_resolved or (now if new_status in ('Resolved', 'Closed') else None)

    # escalated is a permanent historical flag — once set, never cleared
    cur_esc    = row['escalated']
    cur_esc_at = row['escalated_at']
    new_esc    = 1 if (new_status == 'Escalated' or cur_esc) else 0
    new_esc_at = cur_esc_at or (now if new_status == 'Escalated' else None)

    modify_db(
        'UPDATE complaints SET status=?, resolved_at=?, escalated=?, escalated_at=? WHERE id=?',
        [new_status, resolved_at, new_esc, new_esc_at, cid]
    )
    log_action(cid, f'Status→{new_status}', session['user_id'], notes)
    flash(f'Complaint updated to {new_status}.', 'success')
    redirect_url = request.form.get('redirect') or request.referrer or url_for('official_dashboard')
...
def admin_dashboard():
    ...
    closed_count = sum(1 for c in complaints if c.status=='Closed')
    escalated_status = sum(1 for c in complaints if c.status=='Escalated')
    stats = {'total':total,
             'pending':sum(1 for c in complaints if c.status=='Pending'),
             'in_progress':sum(1 for c in complaints if c.status=='In Progress'),
             'escalated':sum(1 for c in complaints if c.escalated),
             ...
             'closed':closed_count,
             'resolved_all':resolved_count + closed_count,
             'violated':violated,
...
def complaint_detail(complaint_id):
    row = query_db('SELECT * FROM complaints WHERE complaint_id=?',[complaint_id],one=True)
    if not row:
        flash('Complaint not found.','warning')
        return redirect(url_for('index'))
    complaint = wrap(row, one=True)
    user_info = query_db('SELECT name,email,district FROM users WHERE id=?',[complaint.user_id],one=True)
    officer_info = query_db('SELECT name,email,department FROM users WHERE id=?',[complaint.assigned_to],one=True) if complaint.assigned_to else None
    logs = query_db('''SELECT a.*,u.name as uname FROM audit_logs a
                       LEFT JOIN users u ON a.performed_by=u.id
                       WHERE a.complaint_id=? ORDER BY a.timestamp DESC''',[complaint.id])
    return render_template('complaint_detail.html', complaint=complaint,
                           user_info=user_info, officer_info=officer_info, logs=logs)
...
def admin_users():
    ...
    for u in users:
        uid = u['id']
        total = query_db('SELECT COUNT(*) FROM complaints WHERE user_id=? OR assigned_to=?',[uid,uid],one=True)[0]
...
def toggle_user_role(uid):
    ...
    modify_db('UPDATE users SET role=? WHERE id=?',[new_role, uid])
